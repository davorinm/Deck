...
class GameViewModel {
    ...
    init(gameMode: GameMode, deck: Deck, table: Table) {
        self.deck = deck
        self.table = table
        
        // GameService state
        let gameService: GameService = ServiceLocator.current.get()
        gameService.state.subscribe(self) { (state) in
            switch state {
            case .initialized:
                break
            case .assembling(gameMode: let gameMode):
                break
            case .prepared(gameType: let gameType):
                break
            case .playing(playingState: let playingState):
    ...
    private func handleGamePlayingState(_ gamePlayingState: GamePlayingState?) {
        ...
        case .playerMove(let player, let card, let completion):
            self.table.addCard(card, player: player, completion: completion)
